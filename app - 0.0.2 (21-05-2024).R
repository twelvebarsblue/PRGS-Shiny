                      ...
                      column(6, style = "padding:0px",
                             box(width = 12, #height = "610px",
                                 ...
                                 column(12, style="margin-top: 2px; text-align:right", 
                                        column(10, style="position:absolute;top:-18px;right:7em", conditionalPanel(condition = "input.go1", downloadButton(style = "color:black;background-color:lightgray;font-size:12px;", "downloadData", "Download"))),
...
server <- function(input, output) {
  ...
  monte_carlo <- eventReactive(input$go1, {
    ...
    runSim <- function(nsamples, minprob, maxprob, crcl, age, sim_time, ii, min_dose, max_dose, resolution, llim, ulim) {
      ...
      combination <- as.data.frame(expand.grid(CRCL = crcl, AGE = age, 
                                               ...
                                               II_1 = ii, II_2 = ii, 
                                               TIME_1 = TIME_1, TIME_2 = TIME_2, TIME_3 = TIME_3,
                                               UNTIL = sim_time))
      ...
      withProgress(message = "Simulating ", value = 0, {
        ...
        for (i in 1:n) {
          ...
          if (all(dv$Median_C > llim) & max(sum_stat$Median_C) < ulim) {
            ...
            cleaned_table <- sum_stat %>% select(time, Median_C, Median_AUC)
            
            colnames(cleaned_table) <- c("Time (h)", "Concentration (mg/mL)", "AUC")
    ...
    return(list(helpText = helpText, crcl = crcl, TIME_1 = TIME_1, TIME_2 = TIME_2, TIME_3 = TIME_3, initDose = initDose, dose2 = dose2, dose3 = dose3, dose4 = dose4, plot_pk = plot_pk, cleaned_table = cleaned_table))
  })
    ...
    runSim <- function(nsamples, crcl, age, sim_time, ii, min_dose = 100, max_dose = 500, resolution = 100, llim, ulim) {
      ...
      withProgress(message = "Simulating ", value = 0, {
        ...
        for (i in 1:nrow(combination)) {
          ...
          dv = filter(out, time %in% c(TIME_1, TIME_2, TIME_3))
  ...
  manual <- eventReactive(input$go3, {
    ...
    runSim <- function(nsamples, minprob, maxprob, crcl, age, d1, d2, d3, d4, t1, t2, t3, sim_time, ii, minpro, llim, ulim) {
      ...
      df <- data_frame(CRCL = crcl, AGE = age, 
                          DOSE_1 = d1, DOSE_2 = d2, DOSE_3 = d3, DOSE_4 = d4,
                          II_1 = ii_1, II_2 = ii_2, 
                          TIME_1 = t1, TIME_2 = t2, TIME_3 = t3,
                          UNTIL = sim_time)
    ...
    runSim(nsamples = input$nsamples, minprob = input$minprob, maxprob = input$maxprob, 
           ...
           d1 = input$init_dose, d2 = input$d1, d3 = input$d2, d4 = input$d3,
           t1 = input$time_1, t2 = input$time_2, t3 = input$time_3,   
           sim_time = input$simtime, ii = input$ii,
           llim = input$Llim, ulim = input$Ulim
          )
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(file) {
      ...
      file.copy("./Monte Carlo/Monte-Carlo.Rmd", tempReport, overwrite = TRUE)
      file.copy("./Monte Carlo/logo.png", tempLogo, overwrite = TRUE)
      file.copy("./Monte Carlo/cover.png", tempCover, overwrite = TRUE)
 ...
 # Monte-Carlo
  observeEvent(monte_carlo(),{
    output$clearance <- renderInfoBox({
      infoBox(
        "Clearance", paste0(round(monte_carlo()$crcl), " mL/min"), icon = tags$i(class = "fas fa-hospital-user", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$initDose <- renderInfoBox({
      infoBox(
        "Initial Dose", paste0(round(monte_carlo()$initDose), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose2 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", monte_carlo()$TIME_1, "h"), paste0(round(monte_carlo()$dose2), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose3 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", monte_carlo()$TIME_2, "h"), paste0(round(monte_carlo()$dose3), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose4 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", monte_carlo()$TIME_3, "h"), paste0(round(monte_carlo()$dose4), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
  })
  ...
  observeEvent(manual(),{
    output$clearance <- renderInfoBox({
      infoBox(
        "Clearance", paste0(round(manual()$crcl), " mL/min"), icon = tags$i(class = "fas fa-hospital-user", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$initDose <- renderInfoBox({
      infoBox(
        "Initial Dose", paste0(round(manual()$initDose), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose2 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", manual()$TIME_1, "h"), paste0(round(manual()$dose2), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose3 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", manual()$TIME_2, "h"), paste0(round(manual()$dose3), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose4 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", manual()$TIME_3, "h"), paste0(round(manual()$dose4), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
  })
  ...
  observeEvent(bayes(),{
    output$clearance <- renderInfoBox({
      infoBox(
        "Clearance", paste0(round(bayes()$crcl), " mL/min"), icon = tags$i(class = "fas fa-hospital-user", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$initDose <- renderInfoBox({
      infoBox(
        "Initial Dose", paste0(round(bayes()$initDose), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose2 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", bayes()$TIME_1, "h"), paste0(round(bayes()$dose2), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose3 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", bayes()$TIME_2, "h"), paste0(round(bayes()$dose3), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
    output$dose4 <- renderInfoBox({
      infoBox(
        paste0("Dose at ", bayes()$TIME_3, "h"), paste0(round(bayes()$dose4), " mg"), icon = tags$i(class = "fas fa-prescription", style="font-size: 12px"),
        color = "purple"
      )
    })
  })
